...
Shader "MyShader/09-LightImprove/AttentionAndShadowUseBuildInFunctions"
{
    Properties
    {
    ...
    SubShader
    {
        Pass {
            ...
            struct v2f {
                ...
                SHADOW_COORDS(2)   // 内置宏，声明一个用于对阴影纹理采样的坐标，参数为下一个可用级插值寄存器的索引值
            };
            ...
            v2f vert(a2v v) {
                ...
                TRANSFER_SHADOW(f);   // 返回另一个内置宏，用于在顶点着色器中计算之前声明的阴影纹理坐标
                return f;
            }
            ...
            fixed4 frag(v2f f) : SV_Target {
                ...
                UNITY_LIGHT_ATTENUATION(atten, f, f.worldPos);   // 用于计算光照阴影的宏，该宏会为我们声明 atten 变量
